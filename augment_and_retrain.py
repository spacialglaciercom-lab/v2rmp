...
"""
Augment training data via feature-level SMOTE + noise, then retrain all models.
...
def get_args():
    ...
    p.add_argument("--n-augment", type=int, default=2, help="SMOTE neighbors to generate per minority instance")
    p.add_argument("--min-per-class", type=int, default=150, help="Minimum instances per class after augmentation")
...
def augment_smote(entries: list, min_per_class: int, n_neighbors: int, noise_std: float = 0.03):
    """
    SMOTE-style synthetic instance generation for minority classes.
    For each class below min_per_class, generate synthetic instances by:
    1. Picking a random instance from that class
    2. Picking k random neighbors from same class
    3. Interpolating features + adding Gaussian noise
    """
    ...
    for c in range(NUM_SOLVERS):
        ...
        for _ in range(needed):
            # Pick random seed instance
            i = rng.integers(0, current)
            seed_feat = X_cls[i]
            # Pick random neighbor from same class
            j = rng.integers(0, current)
            neighbor_feat = X_cls[j]
            # Interpolate
            alpha = rng.random()
            synthetic = alpha * seed_feat + (1 - alpha) * neighbor_feat
            # Add small Gaussian noise
            synthetic += rng.normal(0, noise_std, size=synthetic.shape).astype(np.float32)
            synthetic = np.clip(synthetic, 0.0, 1.0)

            # Derive augmented entry — keep solver_dists but mark gap differently
            base_entry = cls_entries[i]
            aug_entry = copy.deepcopy(base_entry)
            aug_entry["features"] = synthetic.tolist()
            aug_entry["augmented"] = True
            # Perturb distances slightly to reflect feature change
            solver_dists = aug_entry["solver_dists"]
            perturbed = []
            for sd in solver_dists:
                d = float(sd["distance"]) * (1.0 + rng.normal(0, 0.02))
                perturbed.append({"solver": sd["solver"], "distance": max(d, 1.0)})
            # Re-label: best solver based on perturbed distances
            best = min(perturbed, key=lambda x: x["distance"])

            # Bias: with 70% probability keep original class, with 30% switch to new winner
            # This prevents the model from always predicting the augmented class
            if rng.random() < 0.7:
                best_solver = SOLVER_IDS[c]
            else:
                best_solver = best["solver"]

            aug_entry["best_solver"] = best_solver
            aug_entry["best_distance_km"] = best["distance"]
            aug_entry["solver_dists"] = perturbed
            aug_entries.append(aug_entry)
...
def prepare_labels(entries: list):
    n = len(entries)
    X = np.zeros((n, 28), dtype=np.float32)
...
def focal_loss(logits, targets, weights, gamma=2.0, label_smooth=0.06):
...
class BalancedBatchSampler(torch.utils.data.Sampler):
    def __init__(self, labels, batch_size, oversample_minority=True):
        self.labels = labels
...
def train_classifier_focal(name, X, Y, hidden_dims, epochs=400, lr=1e-3,
...
def main():
    ...
    print("=" * 60)
    print("v2rmp Augmented + Rebalanced Training Pipeline v3")
    print("=" * 60)

    print(f"\nLoading training data from {args.data}...")
    entries = load_entries(args.data)
    print(f"  {len(entries)} raw instances")
    ...
    print(f"\n  Augmenting with SMOTE (n_neighbors={args.n_augment}, min_per_class={args.min_per_class})...")
    ...
    X, Y_label, Y_quality, Y_automl, moves = prepare_labels(aug_entries)
