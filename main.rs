...
fn main() -> Result<()> {
    ...
    // Initialize clipboard (optional, graceful fallback if unavailable)
                    ...
                    match key.code {
                        ...
                        KeyCode::Char('c') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                            ...
                            if let Some(ref mut clip) = clipboard {
                                if let Ok(()) = clip.set_text(&app.input_mode.buffer) {
                        ...
                        KeyCode::Char('a') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                            // Select all (not implemented in TUI, but we can copy all)
                            if let Some(ref mut clip) = clipboard {
                                if let Ok(()) = clip.set_text(&app.input_mode.buffer) {
                                    app.log(LogLevel::Success, "Copied all to clipboard");
                                }
                            }
                        }
                        KeyCode::Char(c) => {
...
fn handle_enter(app: &mut App) {
    match app.current_view {
        ...
        View::Extract => {
            // Start input for bounding box
            app.start_input(InputField::BoundingBox);
        }
        View::Compile => {
            // Start input for input file
            app.start_input(InputField::InputFile);
        }
        View::Optimize => {
            // Start input for cache file
