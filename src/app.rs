...
impl App {
    ...
    pub fn close_file_browser(&mut self, selected_path: Option<PathBuf>) {
        ...
        if let (Some(path), Some(field)) = (selected_path, target_field) {
            let path_str = path.to_string_lossy().to_string();
            self.update_field(&field, path_str);

            self.current_view = match field {
                InputField::InputFile | InputField::OutputFile => View::Compile,
                InputField::CacheFile | InputField::RouteFile => View::Optimize,
                InputField::CleanInputFile | InputField::CleanOutputFile => View::Clean,
                InputField::VrpWaypointsFile => View::Vrp,
                _ => View::Home,
            };
        } else {
    ...
    pub fn update_field(&mut self, field: &InputField, value: String) {
        match field {
    ...
    pub fn confirm_input(&mut self) {
        ...
        let field = self.input_mode.field.clone();
