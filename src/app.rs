    ...
    pub fn close_file_browser(&mut self, selected_path: Option<PathBuf>) {
        if let Some(browser) = self.file_browser.take() {
            if let Some(path) = selected_path {
                ...
                self.handle_file_browser_selection(browser.target_field, path_str);
            } else {
                // User cancelled — restore the view they came from
                self.current_view = browser.previous_view;
            }
        }
    }

    fn handle_file_browser_selection(&mut self, target_field: InputField, path_str: String) {
        match target_field {
