...
mod tests {
    ...
    fn test_parse_bbox_success() {
        ...
        let result = parse_bbox(&args).unwrap();
        assert_eq!(result.min_lon, -74.0);
        assert_eq!(result.min_lat, 40.0);
        assert_eq!(result.max_lon, -73.0);
        assert_eq!(result.max_lat, 41.0);
    }

    #[test]
    fn test_parse_bbox_missing_bbox() {
        let args = json!({});
        let result = parse_bbox(&args);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "Missing 'bbox' parameter");
    }

    #[test]
    fn test_parse_bbox_missing_fields() {
        let missing_min_lon = json!({
            "bbox": {
                "min_lat": 40.0,
                "max_lon": -73.0,
                "max_lat": 41.0
        ...
        let result = parse_bbox(&missing_min_lon);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "bbox.min_lon required");

        let missing_min_lat = json!({
            "bbox": {
                "min_lon": -74.0,
                "max_lon": -73.0,
                "max_lat": 41.0
        ...
        let result = parse_bbox(&missing_min_lat);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "bbox.min_lat required");

        let missing_max_lon = json!({
            "bbox": {
                "min_lon": -74.0,
                "min_lat": 40.0,
                "max_lat": 41.0
        ...
        let result = parse_bbox(&missing_max_lon);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "bbox.max_lon required");

        let missing_max_lat = json!({
            "bbox": {
                "min_lon": -74.0,
                "min_lat": 40.0,
                "max_lon": -73.0
        ...
        let result = parse_bbox(&missing_max_lat);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "bbox.max_lat required");
    }

    #[test]
    fn test_parse_bbox_invalid_types() {
        ...
        let result = parse_bbox(&invalid_type);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().to_string(), "bbox.min_lon required");
    }
}
