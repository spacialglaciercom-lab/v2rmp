...
/// Calculate the initial bearing from point 1 to point 2 in degrees (inputs in radians).
fn bearing_rad(lat1_r: f64, lon1_r: f64, lat2_r: f64, lon2_r: f64) -> f64 {
    ...
    let bearing_r = x.atan2(y);
    bearing_r.to_degrees().normalize(0.0, 360.0)
}
...
pub fn solve_cpp(
    ...
    // Pre-calculate radian coordinates for all nodes to avoid redundant conversions
        ...
        while !forward_done || !backward_done {
            if !forward_done {
                ...
                if (v_lat - u_lat) * METERS_PER_LAT_DEGREE >= best_dist {
                    ...
                    let dist = super::haversine_m_rad(u_lat_r, u_lon_r, v_lat_r, v_lon_r);
                    if dist < best_dist {
                        best_dist = dist;
                        best_v = Some(v);
                    }
    ...
    let start_node = if let Some((dep_lat, dep_lon)) = depot {
        let mut best_node = 0;
        let mut best_dist = f64::MAX;
        let dep_lat_r = dep_lat.to_radians();
        let dep_lon_r = dep_lon.to_radians();
        for (i, &(n_lat_r, n_lon_r)) in nodes_rad.iter().enumerate() {
            let dist = super::haversine_m_rad(dep_lat_r, dep_lon_r, n_lat_r, n_lon_r);
            if dist < best_dist {
                best_dist = dist;
                best_node = i;
            }
    ...
    // Turn classification: reuse bearings between adjacent segments to reduce calculation by 50%
    if circuit.len() > 2 {
        let mut prev_bearing_out: Option<f64> = None;
        for i in 1..circuit.len().saturating_sub(1) {
            ...
            let b_in = prev_bearing_out.unwrap_or_else(|| {
                let (p_lat_r, p_lon_r) = nodes_rad[prev];
                let (c_lat_r, c_lon_r) = nodes_rad[curr];
                bearing_rad(p_lat_r, p_lon_r, c_lat_r, c_lon_r)
            });

            let (c_lat_r, c_lon_r) = nodes_rad[curr];
            let (n_lat_r, n_lon_r) = nodes_rad[next];
            let b_out = bearing_rad(c_lat_r, c_lon_r, n_lat_r, n_lon_r);
            prev_bearing_out = Some(b_out);
        ...
        for route in routes {
            if route.len() > 2 {
                ...
                for i in 1..route.len() - 1 {
                    ...
                    let curr = &route[i];
                    let next = &route[i + 1];
                    let b_in = prev_bearing_out
                        .unwrap_or_else(|| bearing(prev.lat, prev.lon, curr.lat, curr.lon));
                    let b_out = bearing(curr.lat, curr.lon, next.lat, next.lon);
                    prev_bearing_out = Some(b_out);
                    let b_in_rev = (b_in + 180.0).normalize(0.0, 360.0);
